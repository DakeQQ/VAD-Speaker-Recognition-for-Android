...
static inline std::vector<std::pair<int, std::vector<float>>> FBanks(float sp_rate, int fft_points, int fft_points_half, int n_mels) {  // FB-29 Skowronsky-Harris
    float fft_bin_width = sp_rate * inv_fft_points_vad;
...
static inline void BluesteinConvolution(std::vector<std::complex<float>>& data, bool inverse) {
    std::vector<std::complex<float>> bluesteinsequence_a(bluesteinConvolution_size_factor_vad, 0.f);
    for (size_t i = 0; i < data.size(); i++) {
        bluesteinsequence_a[i] = std::conj(bluesteinsequence_vad[i]) * data[i];
    }
    Radix2Forward(bluesteinsequence_a);
    for (size_t i = 0; i < bluesteinConvolution_size_factor_vad; i++) {
        bluesteinsequence_a[i] *= bluesteinsequence_b_vad[i];
    }
    Radix2Inverse(bluesteinsequence_a);
    float nbinv = 1.f / static_cast<float>(bluesteinConvolution_size_factor_vad);
    for (size_t i = 0; i < data.size(); i++) {
        data[i] = nbinv * std::conj(bluesteinsequence_vad[i]) * bluesteinsequence_a[i];
    }
}
...
static inline int Compute_Features(const int n_mels, const int fft_points, const int target_amount_features, const float inv_factor, bool noise, bool dc_offset, std::vector<float> &wave_points, const std::vector<std::pair<int, std::vector<float>>> &fbank, std::vector<float> &save_fearures, bool use_Radix2Inverse) {
    ...
    for (int i = 0; i < target_amount_features; i++) {
        ...
        adding_window(fft_data,Blackman_Harris_factor_vad);
...
Java_com_example_myapplication_MainActivity_Load_1Models_1B(JNIEnv *env, jobject clazz,
    ...
    OrtSessionOptions *session_options_B;
    {
        ...
        ort_runtime_B->AddSessionConfigEntry(session_options_B,
                                             "session.intra_op_thread_affinities",
                                             "6;8");  // We set two small-core here, although one small-core is fast enough for ERes2Net model.
...
Java_com_example_myapplication_MainActivity_Pre_1Process(JNIEnv *env, jobject clazz,
    ...
    for (int i = 0; i < fft_points_vad; i++) {
        Blackman_Harris_factor_vad[i] =
                0.35875f - 0.48829f * std::cosf(static_cast<float> (i) * window_factor_vad) +
                0.14128f * std::cosf(static_cast<float> (i) * 2.f * window_factor_vad) -
                0.01168f * std::cosf(static_cast<float> (i) * 3.f * window_factor_vad);
    }
    fbanks_vad = FBanks(sample_rate_vad, fft_points_vad, fft_points_half_vad, num_bins_vad);
    bluesteinConvolution_size_factor_vad = CelingPowerOf2((fft_points_vad << 1) - 1);
    Radix_factor.resize(12, std::vector<std::complex<float>>(bluesteinConvolution_size_factor_vad,0.f));  //  for 4096, 2^12
    int index = 0;
    for (int i = 1; i < bluesteinConvolution_size_factor_vad; i <<= 1) {
        for (int j = 0; j < bluesteinConvolution_size_factor_vad; j++) {
...
Java_com_example_myapplication_MainActivity_Run_1VAD_1SR(JNIEnv *env, jclass clazz,
    ...
    }
    {
        for (int i = 0; i < amount_of_mic_channel; i++) {
            ...
            Compute_Features(num_bins_vad,fft_points_vad,amount_of_features_compute_per_loop,
                             inv_fft_points_vad,add_noise,remove_dc_offset,resample_signal[i],
                             fbanks_vad, save_features[i],true);
            ...
            if (i != jstop_sr) {
                ...
                int index_vad = 0;
                for (int j = total_elements_in_sliding_window - refresh_size; j < total_elements_in_sliding_window; j++) {
                    features_for_vad[i][j] =
                            (features_for_vad[i][j] + neg_mean_vad[index_vad]) * inv_std_vad[index_vad];
                    index_vad++;
        ...
        for (int k = 0; k < amount_of_mic_channel; k++) {
            if (k != jstop_sr) {
                ...
                for (int i = 0; i < number_of_frame_state; i++) {
                    ...
                    if (sum_score <= one_minus_speech_threshold) { // be simplified. The original expression: exp(log(1 - sum_score)) >= exp(speech_2_noise_ratio * log(sum_score)) + speech_threshold
        ...
        for (int k = 0; k < amount_of_mic_channel; k++) {
            if (trigger_SR[k]) {
                ...
                ort_runtime_B->CreateTensorWithDataAsOrtValue(
                        memory_info,
                        reinterpret_cast<void *>(&(*(features_for_speaker_confirm[k].end() -
                                                     res2net_input_size))), res2net_buffer_size,
                ...
                std::move(reinterpret_cast<float*> (output_tensors_buffer_0), reinterpret_cast<float*> (output_tensors_buffer_0) + model_hidden_size_SC, speaker_vectors.begin() + index_k);
            }
            index_k += model_hidden_size_SC;
        }
    }
    jfloatArray final_results = env->NewFloatArray(total_elements_in_pre_allocate);
    env->SetFloatArrayRegion(final_results, 0, total_elements_in_pre_allocate,speaker_vectors.data());
